...
const Heatmap = ({ heatmapDataset }) => {
  ...
  const init = async () => {
    ...
    const generateDataset = (options = {}) => {
      ...
      const days = new Array(forwardDays)
        ...
        .map((itm, idx) => {
          ...
          const itmTotal =
            config.fill[dayjs().subtract(idx, 'days').format('YYYY-MM-DD')]
          if (itmTotal) {
            return {
              date: itmDate,
              total: itmTotal,
    ...
    const myColor = d3.scaleLinear(
      [0, 5],
      [`${theme.palette.primary.main}40`, theme.palette.primary.main]
    )
    ...
    const mousemove = function (event, d) {
      Tooltip.html(
        d.total ? `${d.date}</br>发布 ${d.total || 0} 篇文章` : d.date
      )
